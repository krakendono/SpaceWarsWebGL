...
{
    [SerializeField] private GameObject projectilePrefab; // The projectile prefab to be shot
    [SerializeField] private Transform[] firePoint; // Get the position rotation at which it will be shot at
    [SerializeField] private float projectileSpeed = 10f; // Set the speed for the projectile
    private float spawnTimer; // The rate of fire for each shot
    [SerializeField] private float spawnInterval = 1.5f; // Set the the rate of fire
    ...
    void Start()
    {
        ...
        if (enemy.currentEnemyLevel == Enemy.EnemyLevel.AggressiveShip)
        {
            spawnInterval = 3f;
        }
        if (enemy.currentEnemyLevel == Enemy.EnemyLevel.SneakShip)
        {
            spawnInterval = 1.5f;
        }
        if (enemy.currentEnemyLevel == Enemy.EnemyLevel.SmartShip)
        {
            spawnInterval = 3f;
        }
    }
    ...
    void Update()
    {
        ...
        if (spawnTimer <= 0f && enemy.currentEnemyLevel == Enemy.EnemyLevel.AggressiveShip)
        {
            BaseShot();
            spawnTimer = spawnInterval;
        }
        ...
        if (spawnTimer <= 0f && enemy.currentEnemyLevel == Enemy.EnemyLevel.SneakShip && enemy.isSneaking)
        {
            BaseShot();
            spawnTimer = spawnInterval;
        }

        if (spawnTimer <= 0f && enemy.currentEnemyLevel == Enemy.EnemyLevel.SmartShip && !enemy.isRunning)
        {
            BaseShot();
            spawnTimer = spawnInterval;
        }
