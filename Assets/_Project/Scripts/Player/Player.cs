...
{
    [SerializeField] private float moveSpeed = 5f; // The player speed
    float thrusterAmount; // Total Player thruster amount
    bool isThrusterOn; // Get if the player using the thrusterS
    Rigidbody rigidBody; 
    Camera mainCamera;
    Vector2 screenBounds; // The screen boundary to lock the player in the visible camera space
    float objectWidth; // The width of the player
    float objectHeight; // The height of the player
    [SerializeField] private GameObject shieldObject; // The shield power up
    [SerializeField] private Animator ship; // The ship animation for turning left and right
    [SerializeField] private Animator shield; // The ship animation for the shield
    [SerializeField] private GameObject damageLeft; // Damage for the left side of ship
    [SerializeField] private GameObject damageRight; // Damage for the right side of ship
    [SerializeField] private AudioSource powerUpSound; // Power up sound
    CameraShake cameraShake; // Get the Camera gameobject and grab the CameraSHake script
    Vector3 pointingTarget; // Target where to point the ship at aka the mouse
    ...
    void Update()
    {
        ...
        // Allow the player to perform a magnetic pull for items when pressing C
        if (Input.GetKeyDown(KeyCode.C))
        {
            GameManager.Instance.isSuction = true;
        }

        // Allow the player to suck in powerups and rewards
        if (Input.GetKeyUp(KeyCode.C))
        {
            GameManager.Instance.isSuction = false;
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        // Kill player instantly if they hit the enemy 1 time unless there is shield
        if (other.tag == "Boss")
        {
            if(shieldAmount > 0)
            {
